...
namespace Blue.Tree
{
    public class MathTree : MonoBehaviour
    {
        ...
        private void UpdateBoxes()
        {
            ...
            if (m_rootNode.nodeState == Node.NodeStates.SUCCESS)
            {
                SetSucceeded(m_rootNodeBox);
            }
            else if (m_rootNode.nodeState == Node.NodeStates.FAILURE)
            {
                SetFailed(m_rootNodeBox);
            }

            /** Update 2A node box */
            if (m_node2A.nodeState == Node.NodeStates.SUCCESS)
            {
                SetSucceeded(m_node2aBox);
            }
            else if (m_node2A.nodeState == Node.NodeStates.FAILURE)
            {
                SetFailed(m_node2aBox);
            }

            /** Update 2B node box */
            if (m_node2B.nodeState == Node.NodeStates.SUCCESS)
            {
                SetSucceeded(m_node2bBox);
            }
            else if (m_node2B.nodeState == Node.NodeStates.FAILURE)
            {
                SetFailed(m_node2bBox);
            }

            /** Update 2C node box */
            if (m_node2C.nodeState == Node.NodeStates.SUCCESS)
            {
                SetSucceeded(m_node2cBox);
            }
            else if (m_node2C.nodeState == Node.NodeStates.FAILURE)
            {
                SetFailed(m_node2cBox);
            }

            /** Update 3 node box */
            if (m_node3.nodeState == Node.NodeStates.SUCCESS)
            {
                SetSucceeded(m_node3Box);
            }
            else if (m_node3.nodeState == Node.NodeStates.FAILURE)
            {
                SetFailed(m_node3Box);
            }
        }

        private Node.NodeStates NotEqualToTarget()
        {
        ...
        private Node.NodeStates AddTen()
        {
            m_currentValue += 10;
            m_valueLabel.text = m_currentValue.ToString();
            if (m_currentValue == m_targetValue)
